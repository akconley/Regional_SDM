...
ranPtsFiles
###Read in a list of the codes you want to run this session and only select those RanPts
###files that match those codes
pull_list<-readLines("D:\\Git_Repos\\PROs\\Files_coded_ready_for_points_6_6.csv")

ranPtsFiles <- unique (grep(paste(pull_list,collapse="|"), 
                         ranPtsFiles, value=TRUE))

len_fileList<-length(ranPtsFiles)
